...
--
-- Хост: 127.0.0.1:3306
-- Время создания: Мар 20 2022 г., 14:18
...
-- Процедуры
--
CREATE DEFINER=`root`@`127.0.0.1` PROCEDURE `getBook` (INOUT `bookNamesList` VARCHAR(4000), IN `publishedYear` INT)  BEGIN
    DECLARE finished INTEGER DEFAULT 0;
    DECLARE currBookName varchar(255) DEFAULT "";

    DEClARE yearBooks 
        CURSOR FOR 
            SELECT books_name FROM books WHERE YEAR(books.book_datee) = publishedYear;
...
CREATE DEFINER=`root`@`127.0.0.1` PROCEDURE `getBooks` (INOUT `bookNamesList` VARCHAR(4000), IN `publishedYear` INT)  BEGIN
    DECLARE finished INTEGER DEFAULT 0;
    DECLARE currBookName varchar(255) DEFAULT "";
...
-- Функции
--
CREATE DEFINER=`root`@`127.0.0.1` FUNCTION `avgPrice` (`yearParam` INT) RETURNS DECIMAL(10,0) RETURN (
