...
SELECT code, price, price*29.51 AS UAН, price*0.91 AS EUR, price*105.10 as RUB FROM `books`;

/*15. Вивести значення наступних колонок: код, ціна, ціна в грн. без копійок, ціна без копійок округлена*/
SELECT code, price, price * 29.51 AS UAН, TRUNCATE(price*29.51,0) AS UAН1, ROUND(price*29.51,0) AS UAН2 FROM `books`; 
